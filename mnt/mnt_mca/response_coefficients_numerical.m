%[RS,RJ,RS2,RJ2] = response_coefficients_numerical(network,c,ind_rel_par)
%
...
function [RS,RJ,RS2,RJ2] = response_coefficients_numerical(network,c,ind_rel_par)
...
RJ2      = nan * zeros(nr,np,np);
RS2      = nan * zeros(nm,np,np);
RJ2diag  = nan * zeros(nr,np);  %  diagonal elements from RJ2
RS2diag  = nan * zeros(nm,np);  %  diagonal elements from RJ2
