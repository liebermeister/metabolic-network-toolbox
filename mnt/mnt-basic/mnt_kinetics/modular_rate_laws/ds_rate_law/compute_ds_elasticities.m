...
saturation_term =  diag(1./D) * [ diag(theta_plus) * Mplus + diag(theta_minus) *  Mminus ];

E.sc_E_plus_c  =  Wplus .* alpha_A - Wminus .* beta_I + Mplus  - saturation_term;
E.sc_E_minus_c =  Wplus .* alpha_A - Wminus .* beta_I + Mminus - saturation_term;
...
for it = 1:nr,
 ...
 E.sc_E_plus_cc(it,:,:) = ...
 - [ beta_M(it,:)' * beta_M(it,:) ] .* ...
        [ [theta_plus(it)  * Mplus(it,:)' * Mplus(it,:) + ...
           theta_minus(it) * Mminus(it,:)'* Mminus(it,:)] ...
          ./ D(it) ...
          - [ [theta_plus(it) * Mplus(it,:)' + theta_minus(it) * Mminus(it,:)'] * ...
              [theta_plus(it) * Mplus(it,:)  + theta_minus(it) * Mminus(it,:)]  ...
              ./ D(it) ^2 ] ];
 end
