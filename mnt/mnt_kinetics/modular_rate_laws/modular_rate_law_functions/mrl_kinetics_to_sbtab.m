function s = mrl_kinetics_to_sbtab(network, kinetics, quantity_info, save_quantities, file_basename)

% s = mrl_kinetics_to_sbtab(network, kinetics, quantity_info, save_quantities, file_basename)

eval(default('file_basename','[]', 'quantity_info', 'data_integration_load_quantity_info','save_quantities','[]'));

if isempty(save_quantities),
  ll = label_names(fieldnames(network.kinetics),quantity_info.Symbol);
  ll = ll(find(ll));
  save_quantities = quantity_info.QuantityType(ll);
end
