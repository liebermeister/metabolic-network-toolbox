...
% table(network.metabolites( setdiff(1:length(network.metabolites),keep)),0)
 ...
 if isfield(network,'MiriamID__urn_miriam_kegg_reaction'), network1.MiriamID__urn_miriam_kegg_reaction = network.MiriamID__urn_miriam_kegg_reaction(keep_actions); end
 ...
 if isfield(network,'graphics_par'), network1=netgraph_make_graph(network1); 
