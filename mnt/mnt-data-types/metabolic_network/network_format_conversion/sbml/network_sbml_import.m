...
for it=1:length(listOfModifiers),
  ...
  for itt = 1:length(listOfModifiers{it});
    ...
    if isfield(listOfModifiers{it}(itt),'sboTerm'),
      ...
      modifier_type{it} = [modifier_type{it} {''}];
    end
  end
end
  ...
  for it2=1:length(modifier{it1}),
    ...
    if mm,
      switch modifier_type{it1}{it2},
        ...
        otherwise,        network.regulation_matrix(it1,mm)= 0; display('  WARNING (network_sbml_import.m): SBML file contains modifier of unknown type; I will ignore it. Please annotate modifiers sboTerm attributes in the SBML file.');
      end
    end
  end
end
