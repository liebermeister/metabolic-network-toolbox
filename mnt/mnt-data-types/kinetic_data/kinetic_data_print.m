...
for it = 1:length(fn),
  ...
  ii  = find(isfinite(xx.median) + isfinite(xx.mean) + isfinite(xx.std) + isfinite(xx.geomstd) + isfinite(xx.lower) + isfinite(xx.upper));
  nii = length(ii);

  T{it} = [{'!Median','!Mean','!Std','!GeomStd','!Lower','!Upper'}; ...
           num2cell([column(xx.median(ii)), ...
                     column(xx.mean(ii)), ...
                     column(xx.std(ii)), ...
                     column(xx.geomstd(ii)), ...
                     column(xx.lower(ii)), ...
                     column(xx.upper(ii))])];
